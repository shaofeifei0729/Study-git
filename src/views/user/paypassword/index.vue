<template>
  <div class="bind-detail padding-big">
    <el-alert type="error">如果您忘记了您的支付密码，可以联系客服进行重置。初始化支付密码时，原支付密码可以忽略。</el-alert>
    <el-form ref="form" :rules="rules" :model="form" label-position="top" label-width="120px" class="padding-big-top">
      <el-form-item label="原支付密码" prop="name">
        <el-input v-model="form.old" placeholder="请输入原支付密码" show-password />
      </el-form-item>

      <el-form-item label="新支付密码" prop="password">
        <el-input v-model="form.password" type="password" placeholder="请输入新的支付密码" show-password />
      </el-form-item>

      <el-form-item label="重复支付密码" prop="confirmPassword">
        <el-input v-model="form.confirmPassword" placeholder="请重复新的支付密码" show-password />
      </el-form-item>
      <!-- 输入短信验证码或者google验证码 -->
  ...
  methods: {
    submit() {
      this.$refs.form.validate(valid => {
        if (valid) {
          // 修改支付密码
          setPayPassword(this.form).then((resp) => {
            this.$message({
    ...
    sms_send() {
      sendSms('18888888888', 'changepaypassword').then(resp => {
        this.btn_wait()
      })
    }
  }
